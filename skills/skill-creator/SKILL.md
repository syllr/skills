...
> ⚠️ 关键澄清:`mkdir -p` 创建的目录就是 skill 的最终存放位置。OpenCode 通过纯文件发现机制加载 skill(扫描 6+ 个已知路径,后发现的同名 skill 覆盖先发现的),不存在 "deploy 到全局" 这一步骤。如果用户要"全局"skill,直接写到 `~/.config/opencode/skills/<name>/` 即可,不要先写到 `.opencode/skills/` 再复制。
