...
> 【指引】本节给出推荐 + ADR 链。推荐结论写入 [TECHNOLOGY-ARCHITECTURE](../TECHNOLOGY-ARCHITECTURE.template.md) §3.1/§4，本调研只做论证；决策记录链到 [ADR](../../common/ADR.template.md)（`见 ADR-xxx`）。决策规则：各维度按主题设权重（例：功能 40% / 性能 25% / 成本 20% / 生态 15%）对候选加权打分；存在一票否决项（如许可证传染性、硬约束不满足）的候选直接淘汰，不参与加权；推荐 = 加权得分最高且无否决项的候选；证据不足的维度显式标注并相应扣分，不模糊带过；若全员被否决则标注【阻塞-需放宽约束】并问用户；示例权重按主题自定义，未定时问用户。
