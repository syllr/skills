...
generation:
  ...
  related: # 关联模板与联动修改
    ...
    TEST-PLAN: "UT 清单见它 §5.1，写法见它 §5.5（引用不复制）"
  ...
  notes: # 生成注意点（怎么生成）
    ...
    - "UT 写法规范见 TEST-PLAN §5.5（本文档不定义 UT 框架/覆盖率/命名，不设 §6 单元测试规范）"
  ...
  checks: # 生成后反向 check
    ...
    - "§3.1 每个选型「依据」列链 `[ADR-NNNN]`（§2.1 前端选型表同规则）；无 §6 单元测试规范（UT 写法见 TEST-PLAN §5.5）；无 §7 相关文档（信息内联）"
