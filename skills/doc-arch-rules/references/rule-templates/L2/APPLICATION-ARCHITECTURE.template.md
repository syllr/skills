...
generation:
  ...
  checks: # 生成后反向 check
    ...
    - "[ ] wc -l docs/L2/APPLICATION-ARCHITECTURE.md < 350（deep-dives 收敛标准 2/4 阈值命中即单列）"
...
> 【与 deep-dives 分工】高复杂度详情下沉 `deep-dives/`（规则见 notes）。
...
应用划分: {
 ...
 b1: { label: "<模块_业务1>"; width: 185; height: 60; class: mod }
 b2: { label: "<模块_业务2>"; width: 185; height: 60; class: mod }
 b3: { label: "<模块_业务3>"; width: 185; height: 60; class: mod }
 b4: { label: "<模块_文件>"; width: 185; height: 60; class: mod }
 b5: { label: "<模块_权限>"; width: 185; height: 60; class: mod }
 a1: { label: "<模块_Agent编排>"; width: 185; height: 60; class: mod }
 a2: { label: "<模块_解析>"; width: 185; height: 60; class: mod }
 a3: { label: "<模块_检索>"; width: 185; height: 60; class: mod }
 a4: { label: "<模块_生成>"; width: 185; height: 60; class: mod }
 a5: { label: "<模块_规则库>"; width: 185; height: 60; class: mod }
 }
...
> 【填写指引】替换为系统实际应用；四分类定义见 §2.2 上方指引（应用[应用]是主体、容器[容器]你拥有、外部系统[外部]别人提供、用户层[Person]上下文展示）。应用内部再展开一层（controller 接入层 / service+domain 业务层 / infra+integration 支撑层，分层规范见 STRUCTURE §3.1）。
>
> 【收敛】应用内部展开主题命中 deep-dives 判定时下沉（规则见 notes）。
...
> - 模块（Module）：L2 应用内部代码组织单元——回答"代码怎么组织"。按知识域/职责划分（宪法 D 系列：高内聚低耦合），不是能力的别名。
>
> 模块命名以 §2.2 应用划分图为基准（防一物多名，宪法通用语言贯穿）；能力 → 领域/聚合 映射是 N:M（多对多），但该映射 见 domain/ 各域文档（入口 DOMAIN-MODEL §3），本节不重复（应用架构只回答"应用内部有哪些模块"，不重复"能力由哪些聚合承载"）。

### 3.1 应用内模块划分（模块 → 应用）

> 【指引】回答"每个应用内部有哪些代码模块"。模块按知识域划分（承载哪些业务逻辑），不按能力划分，命名与 §2.2 应用划分图一致。
...
> 后端示例（按 STRUCTURE §3.1 固定分层）：`controller`（接口/网关/鉴权）、`service`（各上下文应用服务：<能力_任务> / <聚合_算力> / <聚合_作品>）、`domain`（service/<业务域>/domain/ 横向层：各上下文聚合/实体）、`infra`（数据访问/缓存）、`integration`（外部对接：<外部_支付A>/<外部_支付B>/<外部_AI>）。
> 前端示例：<模块_工具页>、<模块_作品页>、<模块_用户中心>。
>
> 【收敛】模块（如 <模块_转换>）仅名字级，内部细节命中 deep-dives 判定时下沉（规则见 notes）。
