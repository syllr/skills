...
generation:
  ...
  related: # 关联模板与联动修改
    BUSINESS: "业务能力见 BUSINESS §3，功能变化需联动建模；BUSINESS 能力状态是全部实现视图的建模边界（case A 硬约束，完整定义见 notes）"
  ...
  flow: # 生成流程
    - "扫描（自主）：读 L1 + 目标文档；先核对 BUSINESS §3 能力状态（已实现/待规划），待规划能力所在业务域不建模"
    - "已有 DOMAIN-MODEL → 参考旧文档有效信息，但结构按本模板重建"
    - "按模板生成：§1 概述+子域划分 → §2 业务域划分（2.1 清单 / 2.2 映射图 / 2.3 本域映射关系表 / 2.4 跨聚合服务）→ §3 业务域详情索引（域文档清单表）→ §4 跨业务域契约落地 → §5 Mapper（层间模型映射：模型清单/映射矩阵/字段级规则/禁令/完备性校验）→ §6 通用语言表（概念辨析，按需）"
  ...
  notes: # 生成注意点（怎么生成）
    ...
    - "BUSINESS 能力状态 = 全部实现视图的建模边界（case A 硬约束）：建模前先核对 BUSINESS §3 能力状态——已实现（含本迭代）→ 五份实现文档（DOMAIN/API/APPLICATION/STRUCTURE/openapi）正常建模；待规划 → 全部实现视图不建模不留 stub（DOMAIN 不建 Action/状态机、API 不建端点、APPLICATION 不加模块、STRUCTURE 不加目录、openapi 不留 tag/paths），仅 BUSINESS 保留待规划标注（产品全景含规划）——避免\"BUSINESS 无、实现视图有\"漂移"
  ...
  checks: # 生成后反向 check
    ...
    - "BUSINESS 标「待规划」的能力在全部实现视图（DOMAIN/API/APPLICATION/STRUCTURE/openapi）不建模不留 stub，仅 BUSINESS 保留待规划标注；无「BUSINESS 无、实现视图有」漂移"
...
| 层                   | 模型                | 命名约定                                                     | 定义归属                                      |
| -------------------- | ------------------- | ------------------------------------------------------------ | --------------------------------------------- |
| 接入层（controller） | 请求/响应 DTO       | `<XxxRequest>` / `<XxxResponse>`（每端点独立，不复用同一类） | 代码（controller/schema；由代码导出 openapi） |
...
> 【指引】只登记命名差异、类型转换、枚举映射、敏感字段处理等非平凡规则；全量字段定义以代码 models.py 为真源，接口字段看代码导出的 openapi 产物，此处不重复。
...
- 字段三方对齐：代码 models.py/schema（真源）↔ 域文档方法签名 ↔ openapi 导出产物（接口视图），可 diff 机检（缺失/口径不一致 = 漂移，按宪法 §2.2 第4条修复）
...
> 【指引】收拢跨域通用术语（产品-领域映射术语 + DDD 方法论术语），术语/含义/别称三列；术语定义见 BUSINESS 与各域文档，此处只做辨析不重复定义。建模方法论（9 模式表/决策速查/口诀）不写入实例。
