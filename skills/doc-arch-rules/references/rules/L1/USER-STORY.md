---
description: L1 产品层 文档 USER-STORY 的更新规范——修改 docs/L1/USER-STORY.md 时触发，按模板 frontmatter 的 generation 元数据（scan/ask_user/flow/checks/related）生成或更新该文档；模板全文（含 generation 元数据与 Markdown 正文）见本 rule 下方。
globs:
  - "docs/**/USER-STORY.md"
---
...
**本文档在修改 `docs/L1/USER-STORY.md` 时生效。** 目标：按下方模板生成/更新 `docs/L1/USER-STORY.md`，使其结构符合模板契约，保持 SSOT、不漂移、不遗漏联动。
...
2. **按模板正文生成**：以下方「模板全文」的 Markdown 正文为结构基准，把模板复制为 `docs/L1/USER-STORY.md`，按 `> 【指引】` 填写，**删除 generation 元数据块与全部 `> 【指引】` 说明**（实例不含这两者）。
...
以下是 `USER-STORY` 的完整模板（frontmatter generation 元数据 + Markdown 正文，SSOT，来自 references/templates/L1/USER-STORY.template.md）：
...
generation:
  ...
  ask_user:
    ...
    - 故事优先级/范围有争议时 → 问用户
  reentrant: # 可重入（全量/增量）
    - 全量重生成：收到"生成 USER-STORY" → 从模板 + 扫描需求完整重建
    - 增量修改：已有且符合模板 → 只更新变化故事/角色，保留未变
  tools:
...
> 【指引】用户旅程是故事的**上文**——展示用户端到端体验，从中派生故事。**必选，再简单也要画**（哪怕只有 3 个参与方 5 条消息——旅程是理解"故事在什么场景下发生"的最小上下文）。
>
...
> - **粒度上限**：阶段数 ≤ **7±2**（认知负荷上限）；超过说明项目太大，**按子域拆分**（如"创作流程""商业化流程"各一张），而不是塞进一张无限大的图
...
> | **UI**             | `<页面名>(UI)` 或 `<整体UI名>(UI)` | 系统的前端可见层——产品视角的 UI（具体页面：`AI撰写页(UI)`、`首页(UI)`；骨架图用整体 UI：`AI工具箱(UI)`）。UI 是独立泳道，显示"用户和系统之间的中介" |
...
>
> **三角色 + 完整链路**是 §4 硬性要求（详见 §4 角色语义约定末尾）——骨架图也必须画完整 `用户 → UI → 系统 → UI → 用户` 闭环，禁止只画 `用户 → UI` 的纯前端图。
>
...
> 【填写指引】按角色各画一张（创作者一张、游客一张、管理员一张…）。每张图替换为该项目角色自己的阶段骨架；每个阶段的"完成创作（细节见 US-XXX）"指向对应 User Story——**阶段粒度只到触点，交互细节在 §3 故事详情内展开**。
...
> | 小节组成       | 内容                                                              | 说明                                                              |
> | -------------- | ----------------------------------------------------------------- | ----------------------------------------------------------------- |
> | **涉及 UI**    | 该故事用到的页面/弹窗（入口位置 + `.pen` 引用）                   | 回答"这个故事在哪些界面上发生"——UI 信息跟故事走，不单独建页面清单 |
