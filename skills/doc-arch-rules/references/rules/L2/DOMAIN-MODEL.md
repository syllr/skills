...
**本文档在修改 `docs/L2/DOMAIN-MODEL.md` 时生效。** 目标：按下方模板生成/更新 `docs/L2/DOMAIN-MODEL.md`，使其结构符合模板契约，保持 SSOT、不漂移、不遗漏联动。
...
2. **按模板正文生成**：以下方「模板全文」的 Markdown 正文为结构基准，把模板复制为 `docs/L2/DOMAIN-MODEL.md`，按 `> 【指引】` 填写，**删除 generation 元数据块与全部 `> 【指引】` 说明**（实例不含这两者）。
...
以下是 `DOMAIN-MODEL` 的完整模板（frontmatter generation 元数据 + Markdown 正文，SSOT，来自 references/templates/L2/DOMAIN-MODEL.template.md）：
...
generation:
  ...
  reentrant: # 可重入（全量/增量）
    - 全量重生成：收到"生成 DOMAIN-MODEL" → 从模板 + 扫描业务完整重建
    - 增量修改：已有且符合模板 → 只更新变化领域对象，保留未变
  tools:
