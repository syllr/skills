...
定位：test-tools 是 AI 测试执行的基础设施（小型 Node 测试项目，实例位于 `docs/test/test-tools/`）——测试用例的所有操作，以及用户让 AI 直接查项目数据/对账，都必须经它执行（不写 curl、不裸 SQL、不用其它通道直连被测系统）。工具只取证据/执行操作，断言由 AI 判断。
...
1. 读实例 `docs/test/test-tools/README.md`（工具清单/用法/退出码/环境变量）与 `docs/test/test-tools/.env.<环境名>`（连接参数副本）
