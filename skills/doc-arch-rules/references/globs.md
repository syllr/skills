...
> 本文件是 `globs` 的用法说明：写在哪、怎么写、怎么匹配、基线与扩展各怎么动。执行 globs 目录同步（docs-align skill 的 globs 自适应阶段）前先读本文件。
...
- 扩展：globs 目录同步（docs-align skill 的 globs 自适应阶段）按 `STRUCTURE` 目录职责与磁盘现状双向同步的项目路径：
