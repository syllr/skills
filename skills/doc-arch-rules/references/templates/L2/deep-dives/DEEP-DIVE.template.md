...
> 【指引】当某个应用内部处理逻辑复杂（多领域实体协作、异步编排、状态推进）时，为该应用单独展开一张内部时序图——参与者是 [DOMAIN-MODEL](../domain/DOMAIN-MODEL.template.md) 的业务领域实体/领域服务（聚合根、实体、领域服务、值对象），不是模块/类/接口。简单应用可省略本节（生成时说明"无复杂内部流程，省略 §2"）。
