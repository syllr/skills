...
generation:
  ...
  notes: # 生成注意点（怎么生成）
    ...
    - §1 总览**必须是 Mermaid `sequenceDiagram`**（不得用 `flowchart` 方框图代替时序图）；**参与者只能是应用/系统**（对齐 APPLICATION-ARCHITECTURE §2.2：前端应用/后端应用/外部服务/容器），**严禁下钻到应用内部模块/接口/组件**
...
> 【原则】① **系统级问题描述**：deep-dive 回答一个具体的系统级问题（如"怎么上传文件并构建向量""怎么编排底稿生成"），从端到端视角描述——不限于单层，跨越 L1-L4；② **格式统一（分层时序图）**：§1 应用级时序图（参与者=应用）→ §2 应用内部流程（参与者=领域实体，复杂应用才画）→ §3 实体状态流转 → §4 涉及接口；③ **代码即真相**：细节一律用 `参见 File:Line` 链到代码，不复制代码、不复制 AGENTS.md；④ **引用不复制**：实体定义在 [DOMAIN-MODEL](../domain/DOMAIN-MODEL.template.md)、接口契约在 [INTEGRATION](../../L3/INTEGRATION.template.md)、API 在 [API](../../L3/API.template.md)；⑤ 图用 **Mermaid**（sequenceDiagram，图规范见 references/diagram-spec.md），无元信息表、无变更记录；⑥ **严禁自创概念**（完整规则见 notes）：deep-dive 是合成文档，参与者/实体/事件/状态/接口一律来自现有项目文档。
...
> 【指引】本节给读者"这个系统级问题跨应用怎么走"的整体认知。**参与者必须是 [APPLICATION-ARCHITECTURE](../APPLICATION-ARCHITECTURE.template.md) §2.2 的应用/系统**（如前端应用、后端应用、外部服务、容器），**不得下钻到应用内部模块/接口/组件**（那些属于 §2 或代码层）。消息标注真实接口/事件名。只画一张应用级时序图。
...
> 【指引】当某个应用内部处理逻辑复杂（多领域实体协作、异步编排、状态推进）时，**为该应用单独展开一张内部时序图**——参与者是 [DOMAIN-MODEL](../domain/DOMAIN-MODEL.template.md) 的**业务领域实体/领域服务**（聚合根、实体、领域服务、值对象），**不是模块/类/接口**。简单应用可省略本节（生成时说明"无复杂内部流程，省略 §2"）。
...
> 【指引】本节列出 deep-dive 涉及的领域实体与领域服务，状态流转见 §3.2。实体名与状态定义以 domain/ 各域文档为准（入口 [DOMAIN-MODEL](../domain/DOMAIN-MODEL.template.md) §3）。

### 3.1 涉及实体

> 【指引】实体/领域服务名**照抄 domain/ 各域文档（入口 DOMAIN-MODEL §3）**（纯中文名，不加英文注音、不加属性数），业务域列取 DOMAIN-MODEL §2.1 业务域名（如 审计项目业务域 / AI 能力底座业务域）；非领域实体对象（跨域接口值对象、技术任务表）不入本表，接口类内容归 §4。
...
> 【指引】本节用 **sequenceDiagram**（时序图）表达：参与者=各实体（对齐 DOMAIN-MODEL），沿时间轴用**自环消息**标出每个实体的状态跃迁。**自环消息两行**：第一行 `event: <领域事件名>`（对齐 domain/ 各域文档「本域领域事件」，如"审计项目已立项"；无对应事件的内部步骤用动作动词），第二行**直接写该实体的状态变化**（`态A → 态B`，参与者即实体，不加 `entity:` 前缀）。**多实体协同不用单一 stateDiagram**（那只能画一个对象的状态机），也不用堆叠节点的 flowchart（时间轴不清晰）。异常/分段说明可用 Note over。
