...
generation:
  ...
  related: # 关联模板与联动修改
    ...
    DEEP-DIVES: L2 根为索引，deep-dives 为详情，S2 SSOT——同一信息只在一处维护，其余详见 deep-dives/tool-task.md#状态机（示例，命名规则见 L2/deep-dives/INDEX.template.md）
  # 需要用户决策的才问（无歧义则不问）
  ask_user:
...
> 【与 deep-dives 分工】本 L2 根文档为**索引**，详情在 `deep-dives/tool-task.md#状态机`（S2 SSOT，示例）——同一信息只在一处维护，其余详见 `deep-dives/tool-task.md#状态机`，引用不复制。命名规则见 `L2/deep-dives/INDEX.template.md`。
...
>
> - `subgraph`（虚线框）= 内部业务域（如 `<业务域 1>`）；框外节点 = 外部系统（如 `<外部系统：第三方 AI/平台>`）
...
> 【指引】领域核心业务规则汇总表（导航索引）。**规则类型**（SBVR 分类）：**结构规则**（对象必须满足的结构约束，如 openid 唯一）/ **行为规则**（操作触发时需满足的条件，IF-THEN，如算力不足拒绝）/ **推导规则**（从其他值推导，如余额 = 入账 - 扣减）。**SSOT**：完整定义见各聚合小节（不变量/操作入口）与 §3.3 跨聚合服务，本表为导航索引，改规则先改聚合小节。规则 ID（R1/Rn）与聚合小节双向引用。
...
> 【详见】转换域仅留 R8/R9 限流规则索引，推理细节详见 `deep-dives/tool-task.md#状态机`（示例，按实际替换）。
...
> 【详见】事件详情详见 `deep-dives/tool-task.md#状态机`（示例，按实际替换）。
...
> 【详见】数据存储/容量详情详见 `deep-dives/tool-task.md#状态机`（示例，按实际替换）。
