...
generation:
  ...
  flow: # 生成流程
    ...
    - 定位文档模式：**integration-contracts/ 是契约 SSOT**（每外部服务一份契约文件）；INTEGRATION.md 是说明书（不重复接口清单/字段），承载总览 + 概览 + 契约目录引用
    - **globs 双触发说明**：本 rule 与 CONTRACT rule 的 globs 都含 `docs/L3/integration-contracts/**`——修改契约文件时两者同时触发，属有意分层：本 rule 管说明书（总览/状态列同步），CONTRACT rule 管契约文件本身（字段/接口）；各自按职责范围更新，不重复生成
    - 契约文件不存在 → 先按扫描源推导接入/鉴权/接口/字段契约，落 integration-contracts/<service>.md，再生成 INTEGRATION.md 说明书
    - 按模板生成：§1 外部集成总览 → §2 每服务概览（引用契约文件，不复制字段）→ §3 契约文件目录
  notes: # 生成注意点（怎么生成）
    ...
    - 集成来源两个维度：①技术外部依赖（TECHNOLOGY §4：基础设施与外部服务清单）+ ②领域外部数据源（DOMAIN-MODEL §5.1：外部数据资产），二者并集去重后落地为契约文件；不一致时以 TECHNOLOGY §4 为准
    ...
    - **一服务一契约文件（case 2）**：`<service>-<name>.md` kebab-case（如 llm-api.md/vector-service.md），接口字段级契约/语义/错误码在契约文件，说明书只引用不复制（S2）
    - **字段术语 SSOT 在契约文件（case 3）**：外部服务契约的字段术语唯一事实源在契约文件，说明书不写字段；跨文件术语冲突以契约文件为准（防 project_id/file_id ↔ kb_id/doc_id 类冲突）
    - **接口清单以契约为唯一事实源（case 4）**：说明书不列接口清单，接口以契约文件为准（与 API 说明书一致：字段/接口查契约，不手抄）
    - **契约状态标注（case 5）**：契约文件含「契约状态（重要）」小节（mock 中 / 已交付 / 已上线），与总览表的契约状态引用一致；mock 实现（如 InMemoryXxxAdapter）切换真实时替换 adapter 业务零改动
    - **文件迁移/删除全仓同步（case 6）**：契约文件迁移/删除必须 grep 全仓同步引用（STRUCTURE.md 目录树 / docs README 索引 / 上游文档引用 / 本 rule 的 globs），残留 = 0；目录结构变更同步 STRUCTURE + README + CONSTITUTION 文档架构表（S6.1 删除章节规范同类）
  checks: # 生成后反向 check
    - "INTEGRATION.md 为说明书模式：不复制契约文件的接口清单/字段详情/错误码（case 3/4 完成判定）"
    - "每外部服务对应一份 integration-contracts/ 契约文件（kebab-case），总览表可逐个定位"
...
> 【指引】本系统调用的第三方服务清单（供应商 + 用途 + 鉴权方式 + 契约文件 + 契约状态）。

| 外部服务 | 用途   | 鉴权方式 | 归属应用     | 契约文件                             | 契约状态                    |
| -------- | ------ | -------- | ------------ | ------------------------------------ | --------------------------- |
| <服务名> | <用途> | <鉴权>   | <归属应用名> | `integration-contracts/<service>.md` | <mock 中 / 已交付 / 已上线> |
...
**契约状态**：<mock 中 / 已交付 / 已上线>（与 §1 总览表 / §3 目录表同源，改契约文件后三处同步）
...
> 【指引】`integration-contracts/` 按外部服务拆分契约文件（一服务一份，kebab-case），字段级契约/语义/错误码以契约为唯一事实源。

| 文件                                 | 外部服务 | 契约状态                    |
| ------------------------------------ | -------- | --------------------------- |
| `integration-contracts/<service>.md` | <服务名> | <mock 中 / 已交付 / 已上线> |

> 【维护】① 新增外部服务 → 先建契约文件再更新本表与 §1/§2；② 契约字段术语变更 → 只改契约文件，说明书不手抄字段；③ 契约文件迁移/删除 → grep 全仓同步引用（STRUCTURE 目录树 / README 索引 / 上游文档），残留 = 0。
