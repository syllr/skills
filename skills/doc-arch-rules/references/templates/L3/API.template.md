...
generation:
  ...
  # 需要用户决策的才问（无歧义则不问）
  ask_user:
    - 协议选型超出默认列表（HTTP/REST 之外的启用协议，或目标语言超出 Go/Java-Spring/Python-FastAPI/TypeScript/Node-Express 默认 5 种）→ 问用户
    - 项目技术栈未探测到（项目尚无代码/配置文件）→ 列出语言选项（Go / Java-Spring / Python-FastAPI / TypeScript / Node-Express）让用户选，生成说明书只写选定语言的 CLI
    - 项目技术栈已探测到 → 不问，说明书只写该语言的 CLI（不写其他语言）
  flow: # 生成流程
    - 扫描（自主）：读 openapi.yaml + paths/* + components/* + domain/ 各域文档（全部 Actions）+ APPLICATION-ARCHITECTURE §2.2/§3.1（应用与模块）+ PRODUCT §2.2 + 目标文档
    - 确定目标语言：探测项目技术栈（package.json/go.mod/pom.xml 等）→ 探测到则用该语言；探测不到则按 ask_user 让用户选（Go/Java-Spring/Python-FastAPI/TypeScript/Node-Express）
  ...
  notes: # 生成注意点（怎么生成）
    ...
    - 目标语言确定：同 flow 探测规则；最终文档只写选定语言的 CLI，不写其他语言（避免文档膨胀 + 与项目无关）
  ...
  checks: # 生成后反向 check
    ...
    - "API.md 端点增删已同步 openapi.yaml（tag/paths/文件尾端点计数一致；删除在 yaml 注释留痕并指向 ADR）"
...
> 【指引】占位 `<§2 选定语言生成命令>`/`<§2 选定语言产物路径>` 按 §2 该语言小节替换，二者指向同一目标语言产物文件（同语言硬约束见模板 generation notes）。
