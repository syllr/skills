...
generation:
  ...
  notes: # 生成注意点（怎么生成）
    ...
    - **§4 时序图术语分层（L1 不打开 SYSTEM 黑盒）**：L1 时序图只描述「用户 → UI → 系统后端」三泳道协作——不出现 L2/L3 术语：`云函数`/`AI 服务`/`数据库` 归 L2、微信支付等具体三方名归 L3、`小程序`/`后台` 不作独立 actor（是 UI+后端组合，拆 `<页面名>(UI)` + `系统`）；黑盒内部展开位置（拆 APPLICATION → APPLICATION-ARCHITECTURE §2.2、技术栈 → TECHNOLOGY-ARCHITECTURE、三方对接 → INTEGRATION、数据存储 → DOMAIN-MODEL §5、状态机/业务规则 → DOMAIN-MODEL §3）——分层边界归 rule，实例正文不重复
