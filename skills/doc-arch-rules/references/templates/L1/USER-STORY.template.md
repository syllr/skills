...
| ------------- | -------------------------------------------- | ------------------------------------------------------------------------------------ |
| <角色_创作者> | 用 <Action_生成> 快速生成一篇<场景_内容生成> | 能力：**<能力_生成类>**；Action：`<Action_生成>`（DOMAIN-MODEL §3）                  |
...
### 3.1 用 <Action_生成> 快速生成一篇<场景_内容生成>
...
- 关联功能：能力 **<能力_生成类>**（PRODUCT §2.1）+ Action `<Action_生成>`（DOMAIN-MODEL §3，1:N——故事跨多个 Action 时顿号分隔列出）
...
>
> **示例**：下方 4.2.1 是「<场景_内容生成>」完整示范——**每个故事小节 =「涉及 UI」+「交互时序图」一体**。其他故事按需挑选语法，不必照抄全部结构。
