...
generation:
  tools:
    ...
    - Mermaid flowchart（§4.2.B 系统外故事流程图）
  related: # 关联模板与联动修改
  ...
  flow: # 生成流程
    ...
    - 按模板生成：§1 角色定义 → §2 故事清单（按角色分组，系统内/系统野外覆盖 PRODUCT 能力 + 系统外故事，无编号列）→ §3 故事详情（按角色分大章节：§3.A 系统内 / §3.B 系统外，每故事一节，系统内含 Connextra + Given-When-Then + 关联，系统外含 Connextra + flowchart）→ §4.1 每系统内角色总旅程 → §4.2 单故事（§4.2.A 系统内时序图 / §4.2.B 系统外 flowchart）
  notes: # 生成注意点（怎么生成）——写法约束只在 rule 定义，正文不重复
    - 用户故事是需求源头，先画旅程（§4）再从痛点派生故事（§3）
    - 角色定义（§1）是组织单位：每个角色一张总旅程图（系统内）+ 一批故事
    - **故事分系统内/系统外两类**：系统内故事（角色在系统内完成）画**三泳道时序图**（§4.2.A），用 Connextra + Given-When-Then AC + 关联；系统外故事（角色无系统入口，如线下用户）**无 Action、不画时野图**，用 **flowchart**（§4.2.B）表达「在系统外做了什么、是什么流程」，用 Connextra + 外部流程 + 驱动/支撑的系统内环节
    ...
    - **写法约束只在 rule 定义，正文不重复**：正文故事只写业务内容（故事/AC/关联），**不逐字重复**「AC 在此定义（SSOT）」声明、「1:N 顿号分隔」「<角色>无系统入口」等写法约束——这些在 rule 定义一次，正文精简提及
  ...
  checks: # 生成后反向 check（按类型分维度）
    ...
    - "§3 每节在 §4.2 有对应小节：系统内 → §4.2.A 时序图；系统外 → §4.2.B flowchart；§4.2 系统内时序图小节数 == 系统内故事数（系统外故事 flowchart 在 §3.B 内）"
...
> 【指引】一表汇总所有故事，让读者 30 秒看清"有哪些用户故事"。按**角色**组织，角色下再按**故事类型**分（系统内/系统外）。功能状态与优先级在 PRODUCT §2 产品能力架构图维护（SSOT），此处不重复。
> **故事类型约定**：系统内故事（角色在系统内完成）→ 有关联 Action，§4.2.A 画三泳道时序图；系统外故事（角色无系统入口）→ 无 Action，§4.2.B 用 flowchart。
> **关联功能列**（系统内）：故事 → PRODUCT §2.1 能力 + DOMAIN-MODEL Action 的**正向关联（SSOT）**，1 个故事可关联多个能力（每个能力可对应多个 Action，1:N 顿号分隔）。**关联说明列**（系统外）：该外部流程驱动/支撑的系统内环节，无 Action。
>
> 【示例】以「<产品名>」为例（下同）。
...
> 【指引】每个故事一节，按**角色**分大章节：系统内（§3.A）与系统外（§3.B）分开。与清单行一一对应。**只写意图 + 边界**，不写实现细节；写法约束（AC SSOT / 1:N / 无系统入口）由 rule 定义，正文精简提及不逐字重复。
...
- 验收（业务 AC）：在此定义（SSOT）；技术验收在 TEST-PLAN（引用 `docs/L4/TEST-PLAN.md`）
...
>
> **§4 角色语义约定（适用于 §4.1 总旅程和 §4.2.A 系统内单故事时序图；§4.2.B 系统外 flowchart 不适用三泳道）**：
>
> L1 时序图表达 **3 类参与者**（三泳道）：
>
> | 参与者类型         | 命名                                                       | 含义                                                                                                                                                              |
...
> 【填写指引】按角色各画一张（<角色_创作者>一张、<角色_访客>一张、<角色_管理>一张…）。每张图替换为该项目角色自己的阶段骨架；每个阶段的"完成创作（细节见 US-XXX）"指向对应 User Story——**阶段粒度只到触点，交互细节在 §3 故事详情内展开**。⚠️ 图中 `autonumber` 是 Mermaid 自动编号（系统渲染），不违反"内容条目无顺序编号"约束。

### 4.2 单故事交互（UI 涉及 + 交互时序）

> 【指引】**硬性要求：§3 故事详情中有多少个故事，本节就有多少个对应小节（4.2.A.1、4.2.B.1…一一对应）**。漏画 = 故事覆盖不完整（§3 有 N 个故事，§4.2 必须有 N 个小节）。按**角色**分大章节：§4.2.A 系统内（三泳道时序图）、§4.2.B 系统外（flowchart）。
...
#### 4.2.B <角色_线下>（系统外 · flowchart）

> <角色_线下>为**系统外角色**（无系统入口），其故事**无用户+UI+系统 三泳道交互**，不画 `sequenceDiagram`；改用 **flowchart** 描述「在系统外做了什么、是什么流程」，不关心他用什么系统实现。与 §4.2.A（系统内时序图）区分。
