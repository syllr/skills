...
generation:
  ...
  related: # 关联模板与联动修改（= 改本文档前必读的文档）
    ...
    TECHNOLOGY-ARCHITECTURE: docs/L2/TECHNOLOGY-ARCHITECTURE.md——技术栈选型（SSOT），技术栈变化需同步测试工具；UT 写法 SSOT 在本文档 §5.4（不从技术架构复制）
  # 需要用户决策的才问（无歧义则不问）
  ask_user:
  ...
  notes: # 生成注意点（怎么生成）
    ...
    - 测试分三类：E2E/集成（测 API/契约/状态机，代码方式）、流程（用户旅程级，Gherkin + 浏览器自动化 + 截图视觉）、UT（单元清单，写法见本节 §5.4）
...
> 【原则】① **测试视角**：测试分类/策略/用例/报告——回答"怎么验证质量"；② **依赖链**：用户故事（需求场景）→ PRODUCT/DOMAIN-MODEL（功能/事件）→ TEST-PLAN（测试计划，从用户故事取场景）；③ **测试分三类**：E2E/集成（API 契约）、流程（用户旅程级 + 截图视觉）、UT（单元清单，写法见本节 §5.4 SSOT）；④ 用例与 PRODUCT/DOMAIN-MODEL/用户故事追溯（引用不复制）；⑤ 无元信息表、无变更记录。
...
> 【指引】只列**要测哪些单元**（模块/函数 + 覆盖目标 + 关联 Action/Event）。**写法（框架/覆盖率/命名/Mock 策略）见本节 §5.4**（SSOT，此处引用不复制）。
...
> UT 编写规范见本节 §5.4（框架/覆盖率/命名/Mock 策略）。
...
> **UT 写法规范在此唯一定义（SSOT）**——本文档 §5.1 只列「要测哪些单元」清单，本节定义「怎么写」（框架/覆盖率/命名/Mock 策略）。原置于技术架构 §6，因单元测试属测试一环，迁至本文档统一承载。
