...
generation:
  ...
  related: # 关联模板与联动修改
    ...
    SECURITY: 密钥分层与注入规范见它 §6；本文档 §6 只承载密钥集中登记
  ...
  notes: # 生成注意点（怎么生成）
    ...
    - 版本标识二选一：SemVer 版本发布记录（按需，见 §7.6）或 BUILD_COMMIT 机制（git HEAD 写入容器 env，容器名即版本，见 §5）——未启用 SemVer 时裁剪 §7.6 版本发布记录节，版本标识在 §5 说明
    - 密钥管理：集中登记见 §6，不落日志、不落前端包；载体二选一——有密钥管理服务用注入，无则按团队约定集中登记于 §6（取值来源见 §6，明文不入库，规范见 SECURITY §6；*.example 恒占位）
  ...
  checks: # 生成后反向 check
    ...
    - "版本标识机制已明确（SemVer 见 §7.6 或 BUILD_COMMIT 见 §5，二选一不遗漏）"
...
> 【指引】先给环境总览表（环境/用途/运行形态/状态），再补 parity 与唯一依赖声明。私有化单机用"官方基准镜像+产物挂载（不构建自制镜像）"示例，云/K8s 项目改对应形态。双形态（standalone 进程直跑 vs dev 全容器）时在下方「依赖」行声明两环境依赖关系（声明口径单源见生成 notes「按环境分章」条，正文不重复措辞）。
...
| 环境       | 用途                           | 运行形态                                   | 状态       |
| ---------- | ------------------------------ | ------------------------------------------ | ---------- |
| standalone | 开发自测、联调                 | 进程直跑：<如 Vite dev + uvicorn>（§4）    | <已启用>   |
...
部署拓扑: {
  ...
  平台: {
    ...
    standalone: {
      ...
      数据依赖: {
        ...
        mysql: { label: "<如 MySQL>"; shape: stored_data; width: 222; height: 72; class: mod }
        ragflow: { label: "<如 RAGFlow>"; shape: stored_data; width: 222; height: 72; class: mod }
      }
    }
  ...
  外部服务: {
    ...
    smtp: { label: "SMTP"; width: 732; height: 70; class: mod }
  }
}
...
> 【指引】部署单元 = APPLICATION-ARCHITECTURE §2.2 应用划分图的容器化落地；按环境区分运行形态（standalone/dev 双列），启动顺序用 compose depends_on 声明式解决则不手写。依赖服务单列 §3.2（唯一依赖声明单源见 §1「依赖」行）。
...
> 【指引】唯一依赖编排组时引用 §1「依赖」行声明（措辞不重复），列出被谁依赖与健康检查。
...
| ------ | ------------------------------------ | ------ | -------------------------------- |
| admin  | <如 SEED_PASSWORD 环境变量注入>      | <角色> | <如 种子用户（scripts/seed.py）> |
...
| 密钥项          | 环境变量/来源       | 取值来源                          | 用途          |
| --------------- | ------------------- | --------------------------------- | ------------- |
| <如 MySQL 口令> | <如 MYSQL_PASSWORD> | <如 密钥管理服务注入，明文不入库> | <如 业务主库> |
| <如 JWT 签名>   | <如 JWT_SECRET>     | <如 密钥管理服务注入，明文不入库> | <如 登录签名> |
| <如 种子口令>   | <如 SEED_PASSWORD>  | <如 团队保管介质引用，明文不入库> | <如 种子登录> |
| （补充）        |                     |                                   |               |
...
| 密钥项           | 环境变量/来源          | 取值来源                          | 用途                  |
| ---------------- | ---------------------- | --------------------------------- | --------------------- |
| <如 MySQL root>  | <如 ragflow 编排 .env> | <如 团队保管介质引用，明文不入库> | <如 业务库所在 MySQL> |
| <如 RAGFlow Key> | <如 RAGFlow 检索接口>  | <如 密钥管理服务注入，明文不入库> | <如 检索鉴权>         |
| （补充）         |                        |                                   |                       |
...
│   └── ragflow/                        7.3.3 RAGFlow 编排（<N> 件套，清单见 README，如有）
...
| `rebuild-ragflow.sh`（如有）                 | RAGFlow 编排一键重建（保数据卷）    | `dev/ragflow/README.md` |
...
| 初始化脚本 | `backend/scripts/seed.py`    | <如 种子数据（读取 SEED_PASSWORD 环境变量）> | <如 后端> |
