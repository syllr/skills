...
> 【占位符约定】`<占位>` 替换为实际业务名；`<实体_用户>` 表示名为用户的实体，替换后删除尖括号。
...
> 【指引】通过工具自动检查宪法 D1-D11（宪法§2.1）的规范。

| 工具                       | 检查什么（含阈值）                     | 对应宪法                                                                |
| -------------------------- | -------------------------------------- | ----------------------------------------------------------------------- |
| ESLint（JS/TS）        | 命名、圈复杂度≤10、参数≤3、方法行≤30   | D1-D11 自动可查子集（命名/复杂度/参数/方法长度）；其余 D 系列靠人工审查 |
| Checkstyle/PMD（Java） | 命名、方法长度≤30、圈复杂度≤10、参数≤3 | D1-D11 自动可查子集（同上）                                             |
| SonarQube              | 重复率、耦合                           | D1-D11 自动可查子集（重复率/耦合维度扫描）                              |
