...
> 【占位符约定】`<A_B>` 表示 A/B 二选一占位（如 `<实体_用户>` = 实体或用户，按项目实际选一），替换后删除尖括号。
...
| 工具                       | 检查什么（含阈值）                     | 对应宪法                                                               |
| -------------------------- | -------------------------------------- | ---------------------------------------------------------------------- |
| **ESLint**（JS/TS）        | 命名、圈复杂度≤10、参数≤3、方法行≤30   | D1-D9 自动可查子集（命名/复杂度/参数/方法长度）；其余 D 系列靠人工审查 |
| **Checkstyle/PMD**（Java） | 命名、方法长度≤30、圈复杂度≤10、参数≤3 | D1-D9 自动可查子集（同上）                                             |
| **SonarQube**              | 重复率、耦合                           | D1-D9 全量扫描（重复率/耦合维度）                                      |
