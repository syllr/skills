...
> 【指引】项目完整目录树（ASCII），标注每个目录/文件的职责。代码与 docs 位置 = STRUCTURE——README/AGENTS 引用此处；分层规则 = 宪法 §3.1（docs/ 子树的目录名遵守该分层，不重复规则）。代码目录自项目根计最多四级（计数原点 = 项目根，`backend/` 计第 1 级；Java 包结构可折算，本质仍在四级内）；前端/后端分离；后端固定 6 个顶层模块目录（`backend/` 下直接子目录 controller/service/infra/integration/reference/test，自项目根计为二级，职责见 §2）。
>
> 注意：以下目录树仅为示例/样板，实际项目请按 §3 组织原则与「填写指引」（树后）迭代出过渡规范，勿照搬。
...
│   │   │   ├── INDEX.md # 索引：文件清单（宪法 §3.2，每文件一句话描述）
│   │   │   └── <name>.md # 单篇 Deep Dive（如 inference-pipeline.md，kebab-case）
│   │   ├── domain/ # 领域模型（总文档 DOMAIN-MODEL.md 兼索引 + 每业务域一文，宪法 §3.2 总文档例外不设 INDEX.md）
│   │   │   ├── DOMAIN-MODEL.md # 领域模型总文档（概述/划分/契约落地/Mapper）
│   │   │   └── <域>.md # 业务域文档（一域一文：实体/约束/领域操作/状态机/本域事件）
│   │   └── research/ # 调研（第三方技术/竞品，候选≥2 且 维度≥3（单候选+多维验证例外））
│   │       ├── INDEX.md # 索引：文件清单（宪法 §3.2，每文件一句话描述）
...
│   │   └── user/ # 按业务域分组（三级）
│   │       ├── <X>Service.java # 应用服务（编排用例/事务/权限）
│   │       └── domain/ # 领域层横向层（四级，聚合根/实体/值对象/领域服务/仓储接口）
│   ├── infra/ # 基础支撑层（基础设施、通用支撑）
│   │   └── db/ # 按支撑类型分组（三级）
│   │       └── <Z>Config.java
│   ├── integration/ # 接入层（对接外部系统/三方）
│   │   └── payment/ # 按外部系统分组（三级，需 ACL 防腐时可再嵌 adapter/ 达四级）
│   │       └── <Y>Client.java
...
> 【填写指引】替换为项目实际目录；每个条目写清职责；目录树随项目演进更新。前端/后端目录名可按语言/框架命名（如 `client/`/`server/`、`web/`/`api/`），但概念对应上述分层；后端 6 个顶层模块目录名可随语言/框架微调（如 Java 用 `controller/service/repository` 折算），但职责对应不变。

---

## 2. 目录职责说明（顶层模块目录必说清）

> 【指引】对关键目录/文件逐一说明：干什么 + 为什么这么放 + 约束。任何顶层模块目录（`backend/` 下直接子目录，自项目根计为二级）都必须在文档中说清「干什么、有什么作用、职责是什么」。非关键/自明的可省略。
...
- ① 最多四级原则（自项目根计）：计数原点 = 项目根，`backend/` 计第 1 级——代码目录层级最多四级：`backend/controller/` 二级（顶层模块目录）、`backend/controller/user/` 三级、`backend/service/<域>/domain/` 四级（业务域嵌套 + 领域层横向层，达上限即合规）；文件不计入层级（如 `<X>Controller.java` 在三级目录下、`domain/` 内实体文件在四级目录下）；Java 包结构可折算，本质仍在四级内——超过四级时通过包名/命名折算，不无限加深。
...
- ⑥ 代码分层细则：前端/后端固定分层、infra vs integration 判别、层间纪律——见下方「§3.1 代码分层细则」；分层是手段不是目的（目标是依赖单向、关注点分离），固定分层避免为"规范/模板"反复加层。

### 3.1 代码分层细则（前端/后端分开，固定分层）

> 分层是手段不是目的：目标是依赖单向、关注点分离。前端与后端是两套独立的分层体系，采用固定分层（不再演进加层）——DDD 分层能覆盖大多数场景，避免为"规范/模板"反复加层。
>
> 依赖总则（前后端通用）：
>
...
> - 目录命名统一小写：分层目录一律小写（`controller/`、`service/`、`infra/`、`integration/`、`service/<域>/domain/`、`pages/`、`components/`、`store/`、`utils/`）
...
后端固定分层（DDD，`backend/` 下顶层 4 个分层模块目录（自项目根计为二级），全小写；另有 reference/test 两个非分层辅助目录，见 §2）：

```
controller  接入层（接参/参数校验/响应封装/翻译 DTO→Domain）
   ↓ 只允许向下依赖
service     业务层（含 domain 横向层）
  ├─ service/<域>/          应用服务（编排用例/事务/权限/协调领域对象）
  └─ service/<域>/domain/   领域层（聚合根/实体/值对象/领域服务/仓储接口——横向层）
   ↑          ↑
infra     integration
支撑层-内部      支撑层-外部
```

> 本图为逻辑分层（表达依赖方向，不是目录树）：落地目录见 §1——`backend/service/<域>/` 三级、`backend/service/<域>/domain/`
> 四级（业务域嵌套 = 逻辑分层的目录落地，自项目根计达 §3 ① 上限四级即合规）。

> domain 层是横向层：在 `service/<域>/` 目录内，被同目录的应用服务（上方）与 `infra`/`integration`（下方）同时依赖，但 `domain` 自身不依赖任何层——它是依赖的汇聚点，只定义接口（依赖倒置的落点）。聚合/实体/值对象/领域服务都归 `backend/service/<域>/domain/`（自项目根计四级）。
>
...
>
> 归属判别："这个东西坏了找谁修？"——找自己修（DB 挂了自行处理）→ `infra`；找别人修（<外部_支付A> 挂了只能等）→ `integration`。两者都通过接口/适配器接入，都翻译成 Domain 模型（ACL 防腐层在 `integration/` 里）。
...
> 【指引】目录结构发生变化时，在此说明当前结构（不写历史——历史归 ADR）。本文档始终反映当前目录现状；漂移时在此记录「原定义 → 新情况 → 处理决策（新建/调整/保持）及用户确认」。
...
- 引入 deep-dives 示例：原 L2 仅索引文档 → 高复杂度主题（如 inference-pipeline）单列详情 → 新建 `docs/L2/deep-dives/`（判定归 rule 生成时执行（2/4 阈值见宪法 §3.1））→ 用户确认
