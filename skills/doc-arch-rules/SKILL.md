...
- **内容纯净（构建方式只在 rule，实例文档只承载业务内容）**：模板正文里 `> 【指引】` / `> 【模板使用指引】` / `> 【填写指引】` 等【】标记行是**生成提示（构建元信息）**，只服务于"怎么填"，**仅存在于本 rule 的「模板」章节，生成/更新实例文档时一律不得写入实例**——实例文档只承载业务内容，不出现任何构建说明。
- **模板正文三层处置（生成/更新实例时对模板每一行判定）**：
  - **剔除**：带【】标记的引导行（`> 【指引】` 等）；含"对应模板 §X 位 / checks 标 N/A / 按 <模板X> 模板 §X 要求 / 按主题实际替换 / 生成后校验"等**生成指令口吻**的表述 → 实例中删掉，或改写为面向读者的业务说明（如"本主题无精度分层，故 §3 承载选型依据"，而非"对应模板 §3 位"）。
  - **改写**：占位符（`<模块_xx>` `<模型_xx>` `<项目名>`）替换为真实业务名称；示例图/示例表换成实际内容，删掉"示例为…其他主题按实际替换"这类填充说明；删除占位示意行（`| ... | — | 按主题补充 |`）。
  - **保留**：面向读者的原则/元信息（"只写意图+边界""代码即真相"、SSOT 声明、"本图为 X 图"图标注、frontmatter 的 title/doc_type/layer）→ 保留，但要转成业务表述，不保留"按模板填写"口吻。
...
- <checks 2>
- ...
- **实例无构建元信息（机器校验）**：`grep -nE '^> (【指引】|【模板使用指引】|【填写指引】|【原则】|【章节】|【示例】|【覆盖范围】)' <target>` 无输出；`grep -nE '对应模板|checks 标 N/A|按.*模板.*要求|按主题实际替换' <target>` 无输出
...
- 「模板」章节**只放剥离 YAML frontmatter 后的 Markdown 正文**（保留 `> 【指引】` 行——注意此为 rule 内生成提示，**供 rule 读者（生成实例的 AI）参考，绝不写入实例文档**；生成实例时按「硬性要求 · 内容纯净/三层处置」剥离）
