...
2. **垂直：子容器在副容器内均匀分布，不能挤在任意一侧**（ELK 实测结论）：
   ...
   **竖条/标签列正确写法（推荐）**：
   ...
   ⚠️ **竖条/标签列宽度必须能容纳子容器 label**：竖条 width 太小 → 子容器（按 label 自然宽）**向右超出竖条右边界**（实测：竖条 width:170、子容器 label"ROI 分析"宽 119 → 子容器右边界超父容器 9px；width 加到 190 → 富余 11px，安全）。规则：**竖条 width ≥ 最宽子容器 label 宽度 + 约 120px（左右 padding 各 60）**。判断方法：渲染后检查子容器右边界是否 ≤ 竖条右边界，超了就加竖条 width（同时重算外层宽度，见 6.9）。
