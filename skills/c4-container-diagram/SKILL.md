...
## 2. 工作流（分诊 → 新建 5 步 / 修改走 §2.1）

### 2.0 分诊（进入 skill 的第一件事：新建 vs 修改 vs 只读）

**判断依据 = 用户意图 + 目标位置状态，两者结合；歧义必须问用户，禁止猜**：

| 用户信号                       | 目标位置信号                 | 分诊结果                                                   |
| ------------------------------ | ---------------------------- | ---------------------------------------------------------- |
| "画/新建/加一张" 等新建词      | 目标位置**无** d2 块         | → **新建模式**（§2 步骤 1-5 全流程）                       |
| "改/调整/更新/优化/重画那张图" | 目标位置**有** d2 块且已指明 | → **修改模式**（§2.1 工作台 extract → 改 → render → sync） |
| "画图" 但位置**已有** d2 块    |                              | 🔴 歧义 → 问："覆盖现有图还是另建一张？"                   |
| "改图" 但未指明哪张 / 位置无块 |                              | 🔴 歧义 → 问："改哪张？（若无现成图则实为新建）"           |
| 位置有**多个** d2 块           |                              | 🔴 歧义 → 问："第几张（图序号）？"                         |
| "看一下/检查图"                |                              | → **只读模式**（§7 验收，不改代码）                        |

**信息收集（分诊前必做）**：Read 目标文档确认：① 文件存在可编辑；② 该位置是否有 d2 块、有几个；③ 用户是否指定了图名/章节/图序号。**未完成分诊（三选一明确）前，禁止写 d2 代码**。
...
1. **定位目标 Markdown 文档（分诊信息已收集，此处确认插入/修改位置）**：用户调用本 skill 时通常会说"在 `docs/architecture.md` 里画一张 C4 Container Diagram"。在分诊（§2.0）基础上，确认插入位置（新建）或目标代码块（修改）。**未确认目标文档前，禁止写 d2 代码**
...
5. **自检**：新建图用 [7.2 节](#72-验收执行一条命令提取--渲染--脚本校验) 手动流程（提取 → 临时 .d2 → `d2 validate` + `d2 render` → `python3 scripts/verify-svg.py`）；**已有图修改一律用 [§2.1 工作台](#21-修改模式对已有图做调整不等于重画) 一条命令**（extract → render → sync，自动校验 viewBox/超界/等宽/圆角）。

完成标准：目标 Markdown 文档中 ` ```d2 ` 代码块已写入/更新；`verify-svg.py` 脚本校验通过；渲染结果与第 3 步确认的 ASCII 架构一致。

### 2.1 修改模式（对已有图做调整，≠ 重画）

> **目标**：第二次起改图要快（每次 3~5 分钟），不是全图重写。核心原则：**权威源 = Markdown 代码块**，改代码永远改文档原文（保持 `\n` 字面量）；临时 .d2 只读用于验证，**禁止**"改临时文件再复制回来"造成双份维护。
...
- **结构变更**（加层/改列数/模块移动）才重走 ASCII 确认（§2 步骤 3）；纯 label/宽度/颜色/箭头调整**直接改**，改完按上一步工作台命令验证闭环
...
2. **垂直：子容器在父容器内均匀分布，不能挤在任意一侧**（ELK 实测结论）：
   ...
   **确定性公式（实测校准）**：
   ...
   C. 多列/分区场景：每个子容器都要显式写 width (含嵌套分区), 不能只写 grid-columns
      (例外: 单列竖条内子容器 width 可设可不设, 见 B)
   ```

   **验证方法**：写代码前先按公式算好（多列居中 / 单列竖条自动包裹），渲染后对照"子容器最右 ≤ 父容器最右"且左右/上下留白大致对称。**如果超界，先回头检查公式（多列子总宽是否 > 父宽−4），再修，不要逐个容器猜。**不对称 = grid 维度写错、width/height 算错、或竖条误写 grid-gap。
...
> ```
>
> **最稳妥**：用 [§2.1 工作台脚本](#21-修改模式对已有图做调整不等于重画)（`d2-workbench.py`）——文件 IO 直写，从机制上杜绝转义问题。
...
**推荐（一条命令闭环，杜绝转义事故，改图首选）**——[§2.1 工作台脚本](#21-修改模式对已有图做调整不等于重画)：
...
cat > "$TMPDIR/d2-$(date +%Y%m%d%H%M%S).d2" <<'EOF'
<提取的 d2 代码>
EOF
