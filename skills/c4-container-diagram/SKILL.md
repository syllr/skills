...
**元素数 < 列数时（实测 ELK v0.8.1）**：如 2 子容器 + `grid-columns: 3`，ELK **整组居中放置、剩余列留白**（实测子1 x=226、子2 x=518，右列空；父 1000 内容宽 976）——不是"挤中间"也不是"靠左"。仍是残缺布局，正确做法是 `grid-columns` 改为实际子容器数（如 2）。
