...
### 1.1 目标：我们要画出的图（对齐铁律，最高优先级）

> 本 skill 的目标是产出**整齐、对称、可读**的架构图。以下铁律是**最终效果标准**——任何语法、任何模板都必须服从它。生成后自检对照本节；不满足铁律 = 不合格，必须调整。
...
**铁律与语法的关系**：铁律靠语法实现——对称靠"子容器 width = 父容器 width − 120"（6.13），均匀分布靠"一维 grid + 不写 grid-gap + 子容器按公式设 height"（6.13），边界靠"width/height 精确计算 + 竖条宽度容纳 label"（6.9/6.13），文本完整靠 6.14 的 trade-off（扩父容器 vs 缩文本）。**先对照本节定目标，再按 6.9/6.13 计算尺寸，最后才写语法。**
...
| 形状         | D2 语法                         | 用途                 |
| ------------ | ------------------------------- | -------------------- |
| 矩形（默认） | 节点名（不写 shape）            | 通用模块             |
| 圆柱体       | `shape: cylinder`               | 数据库（图2/图6）    |
| 圆形         | `shape: circle` / `shape: oval` | 渠道/路由节点（图2） |
| 队列         | `shape: queue`                  | 消息队列/缓冲        |

写法：`节点名: { shape: cylinder }` 或 `节点名.shape: cylinder`。以上 4 种已实测 validate 通过（注意：`shape: database` 不存在，数据库用 `shape: cylinder`）。
...
2. **垂直：子容器在副容器内均匀分布，不能挤在任意一侧**（ELK 实测结论）：
   ...
   **竖条/标签列正确写法（推荐）**：
   ...
   ⚠️ **竖条/标签列：子容器水平居中对称（铁律 2）**：ELK 的 grid 单元格内子容器**左对齐**（实测：子容器 x 固定 = 父 x+60，宽度变化只影响右边界）。所以**水平居中必须手算子容器 width**：
   ...
   - **横排场景（普通层容器，铁律 2 同样适用）**：层容器（width:W）内 `grid-columns: N` 铺子容器时，**每个子容器必须按公式设 width**，否则 ELK 按内容宽排、总宽 < 层宽时**左对齐偏左**（实测：层 width:780、4 个子容器不设 width → 右侧留白 30-40px）：

   ```
   子容器 width = (层容器 width − 内边距 24 − (N − 1) × grid-gap) / N
   ```

   例：层 width:780、gap:8、N=4 → (780−24−24)/4 = 183；层 width:780、gap:12、N=2 → (780−24−12)/2 = 372。**每个子容器都要写 width**（含嵌套分区容器），不能只写 grid-columns。

   对称验证：渲染后检查**第一个子容器左边距 = 最后一个子容器右边距**（相对层容器边界），且中间间距一致。不等 = 有子容器没设 width 或 width 算错。嵌套分区（2×2 子模块）内部同理：分区容器设 width 后，其内部子模块按分区宽度再套公式。
