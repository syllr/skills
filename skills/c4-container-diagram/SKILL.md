...
| 4   | **对称**         | 子容器到副容器**左右边框距离相等**（水平居中）——**铁律**。垂直方向仅单列容器（竖条/标签列）要求上下均匀，普通多列容器不做垂直居中（D2 限制） |
...
**铁律 2（对称）**：子容器到父容器**左右边框的距离必须相等**（水平居中）——这是硬性要求，任何容器必须满足。**垂直方向**：仅在**单列容器**（竖条/标签列，grid-columns:1）场景下要求上下均匀（靠 height 公式，见 §6.13）；**多行/多列容器**（普通层容器）**不做垂直居中要求**（D2/ELK grid 强制列等高、子容器靠顶部，物理上无法垂直居中——§7.2 检查项 5 也确认此限制）。允许"贴顶"，不允许"贴左不贴右"。
...
| 对称（垂直）      | ⚠️ **D2/ELK 无法垂直居中**（grid 强制列等高、子容器靠顶部）。仅单列竖条/标签列可上下均匀：一维 `grid-columns:1` + 不写 `grid-gap` + 子容器按公式设 `height` | 见 [§6.13](#613-尺寸计算规则副容器--子容器核心约束)，普通多行容器不做垂直居中    |
...
> 完整版见 [6.13 尺寸计算规则](#613-尺寸计算规则副容器--子容器核心约束)（含确定性公式/垂直分布/边界约束/trade-off）。本条为基础公式 + 速查表。
...
2. **按 §6.13 公式算出每个容器、每个子容器的 width**：
   - 多列容器（grid-columns:N, N≥2）：`子width = (父宽−4−(N−1)×gap)/N`，**整组自动居中**，条件是 `Σ子宽+(N−1)×gap ≤ 父宽−4`
...
8. **识图 checklist 辅助**（§7.2 固定 prompt）复核配色/形状/文字等脚本无法数值化的项。
...
**判断结论**：直接给用户 PASS/FAIL + 数值。超界/不等宽 → 对照 §6.13 公式重算 width，再渲染重跑脚本。**脚本是权威，识图（7.2）只做辅助**。

### 7.2 识图审查清单（结构化 checklist，辅助项）
