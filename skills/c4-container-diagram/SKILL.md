...
5. **d2 validate**（语法）+ 渲染。
6. **SVG 坐标验证超界**（确定性判断，见下 §7.1）——每个父容器的每个直接子容器，4 边不超父边界。
7. **SVG 坐标验证等宽**：单列容器子容器 左距==右距（=60）；多列容器整组居中（首末距相等）。
8. **识图工具 checklist**（§7.2 固定 prompt）独立复核。

**发现问题的处理**：先对照 §6.13 公式重算（大概率是 width 没按公式），再改代码重跑；3 轮不过回 §3 ASCII 重确认。**不要逐个容器猜**。

### 7.1 SVG 坐标验证（判断超界/等宽的方法论）

**超界判断**（渲染后，解析 SVG 的 `<rect>`）：

```python
# 伪代码: 对每个父容器 c 和其直接子容器 k:
左超 = c.x - k.x                    # >0 说明子超出父左
右超 = (k.x + k.w) - (c.x + c.w)    # >0 说明子超出父右
上超 = c.y - k.y
下超 = (k.y + k.h) - (c.y + c.h)
# 任一 > 0.5px → 超界, 必须修
```
...
左距 = k.x - c.x
右距 = (c.x + c.w) - (k.x + k.w)
# 左距 == 右距 → 等宽; 左距固定 60 而右距更大 → 子宽没按"父宽−120"设
```

**判断结论**（直接告诉用户 PASS/FAIL + 数值，不依赖识图自由描述）。

### 7.2 识图审查清单（结构化 checklist，不要让它自由描述）
