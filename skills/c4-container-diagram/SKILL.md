...
| 9   | **每层显式算宽** | **每一层嵌套（A→B→C→D）都要为子容器显式算 width**（公式 §6.13；唯一例外：单列竖条不设 width，见 §6.13 B）——只算最外层会出现"贴左偏左"。这是本 skill 最核心的实操铁律 |
...
**铁律 2（对称）**：子容器到父容器**左右边框的距离必须相等**（水平居中）——这是硬性要求，任何容器必须满足。**垂直方向**：仅在**单列容器**（竖条/标签列，grid-columns:1）场景下要求上下均匀（靠子容器 height 公式，见 §6.13）；**多行/多列容器**（普通层容器）**不做垂直居中要求**（D2/ELK grid 强制列等高、子容器靠顶部，物理上无法垂直居中）。允许"贴顶"，不允许"贴左不贴右"。
...
2. **上下 Padding**（仅单列竖条适用）：子容器 `height` 按 §6.13 竖条公式（上下内边距 ≈120 含标题区）；普通多行/多列容器**不做垂直居中**（§1.3 铁律 2）。
...
**选项优先级：1 > 2 > 3 > 4**。选完后按 §7 流程"自检" → 渲染 → 重新走 checklist 验证。**任何"接受超界"的选项都是被禁止的**。
...
1. 用户输入**指到了已有 d2 块**——给了位置+图名（"`docs/L1/PRODUCT-SPEC.md` 那个能力分层图"）、贴了代码块、或说"改/调整/更新那张图" → **修改模式**（§2.2 工作台 extract → 改 → render → sync）
2. 用户说**新建词**（画/新建/新增/加一张）且未引用已有块 → **新建模式**（§2 步骤 1-5 全流程；目标位置已有别的图 → 插入新块，**不动旧图**）
3. 用户说**重画/覆盖/重新生成**某图 → **修改模式**（替换该代码块）
4. 用户说"看一下/检查" → **只读模式**（§7 验收，不改代码）
...
> **CRITICAL — BLOCKING（阻塞性要求 #1，新建模式步骤 2）**: 画架构图前，必须先与用户对齐**几层 / 每层哪些模块 / 标签样式**（顶部居中 vs 左侧竖排）。未对齐前，禁止写 d2 代码。

> **CRITICAL — BLOCKING（阻塞性要求 #2，新建模式步骤 3）**: 写 d2 代码块前，MUST 先在对话中以 ASCII 架构图向用户展示图的大体结构（层数 + 每层模块 + 层间连线）。用户未确认前，禁止写 d2 代码。（ASCII 图要素见 [3 节](#3-ascii-架构确认画图前必做)）
...
4. **在目标文档写/改 ` ```d2 ` 代码块**：在步骤 1 确定的插入位置，写入或修改 ` ```d2 ` 代码块。**首行写图名注释（`# 图名`）——这是图的标识，后续定位/修改靠它语义匹配（§2.1），禁止省略**。格式不限：中文优先（如 `# 系统架构图`），用户喜好为准，中英皆可（`# System Architecture` 也行）——关键是有一个可识别的名字。第二行起 `vars: { d2-config: { layout-engine: elk } }`（默认 elk），可继续写视角/用途等元信息注释。**写完后渲染由 Markdown 引擎自动完成，AI 不做任何输出/渲染动作**
5. **自检**：一律用 [§2.2 工作台](#22-修改模式对已有图做调整不等于重画) 一条命令（自动校验 viewBox/超界/等宽/圆角）——新建图：`extract docs.md --name 图名` 提取刚写的块验证（不需 sync）；已有图修改：`extract → render → sync`。工作台不可用时才用 [7.2 节](#72-验收执行一条命令提取--渲染--脚本校验) 手动流程。
...
- **⚠️ 修改模式豁免 §2 的阻塞性 #1/#2**：对齐参数（阻塞性 #1）和 ASCII 确认（阻塞性 #2）**仅在结构变更（加层/改列数/模块移动）时重走**；纯 label/宽度/颜色/箭头调整直接改，改完工作台命令验证闭环
...
- **改哪层只重算哪层的 width**（§6.13 公式）：未动层/容器保持原值，不因"一致性"重算全图——改 label 就只改 label，改颜色就只改颜色
- 加容器/层时先查是否有同语义边（避免重复边），新容器记得 `border-radius`（§4.8）
- **`\n` 字面量铁律**：label 内换行写 `\n`（反斜杠 + n 两个字符），**禁止**真实换行；用脚本写 .d2 时禁止 `echo`/未加引号 heredoc/Python 字符串拼接（见 §6.17）——工作台脚本天然规避（文件 IO 直写）
...
服务层: {
  ...
  通信组件: { grid-columns: 1; grid-gap: 8; style.font-color: "#1e293b"; style.border-radius: 8   # 分区 1：内部嵌套
    # 注意: 分区不设 width 时, ELK 按内容定列宽(§6.2), 分区内子容器 width 是"内容宽"非撑满公式值
...
2. **垂直：子容器在父容器内均匀分布，不能挤在任意一侧**（ELK 实测结论；垂直居中仅单列容器要求，见 §1.3 铁律 2）：
   ...
   **确定性公式（实测校准）**：
   ...
   C. 多列/分区场景：每个子容器都要显式写 width (含嵌套分区), 不能只写 grid-columns
      ...
      (例外2: 嵌套分区容器本身可不设 width——按内容定列宽 §6.2/§5.5，
       但其内部子容器仍需显式 width)
   ```

   **验证方法**：写代码前先按公式算好（多列居中 / 单列竖条自动包裹），渲染后对照"子容器最右 ≤ 父容器最右"且左右/上下留白大致对称。**如果超界，按 §6.13 超界排查序定位根因（第一嫌疑 label 撑宽 → 第二列数 N → 最后才验公式），再修，不要逐个容器猜。**不对称 = grid 维度写错、width/height 算错、或竖条误写 grid-gap。
...
子容器 width 按公式算出后放不下文本（文本溢出/被截断）时，**不要自己偷偷改布局**，按 §1.6 的 4 选项流程处理（缩 label → 改布局 → 扩父容器 → 消减子项数，优先级 1>2>3>4，详见 §1.6）。
...
- **禁止 `grid-column-span`**（渲染蓝色"2"徽章 + 直角矩形，违反 §4.8 铁律）
...
**验收必须检查 viewBox**：渲染后按 §7.2 检查 viewBox 是正数且合理（如 `0 0 1000~3000 500~1500`）；出现负数大值（`-9e18`）即溢出，按上述规避修正。**d2 validate 通过不代表 viewBox 正常——必须看渲染产物**。
...
> ```
>
> **最稳妥**：用 [§2.2 工作台脚本](#22-修改模式对已有图做调整不等于重画)（`d2-workbench.py`）——文件 IO 直写，从机制上杜绝转义问题。
...
7. **脚本 FAIL** → 按 §6.13 超界排查序定位根因（label 撑宽/N 取错/公式），改代码重渲染重跑，直到脚本全过（最多 3 轮）。
8. **识图 checklist 辅助**（§7.3 固定 prompt）复核配色/形状/文字等脚本无法数值化的项。

**发现问题的处理**：先按 §6.13 超界排查序定位根因（label 撑宽第一嫌疑，非公式错），再改代码重跑；3 轮不过回 §3 ASCII 重确认。**不要逐个容器猜**。
...
左距 = k.x - c.x
右距 = (c.x + c.w) - (k.x + k.w)
# 左距 == 右距 → 等宽（竖条不设 width 时左右都是 60）; 右距 ≈ 0 → 竖条误设 width（v0.8.1 贴右, 见 §6.13 B）
```
...
**判断结论**：直接给用户 PASS/FAIL + 数值。超界/不等宽/无圆角 → 按 §6.13 超界排查序定位根因（label 撑宽 → N → 公式），再渲染重跑脚本。**脚本是权威，识图（7.3）只做辅助**。
...
**⚠️ viewBox 检查（渲染后必做，坑 §6.16）**：工作台脚本自动检查；手动流程用：
