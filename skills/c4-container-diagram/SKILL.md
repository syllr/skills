...
> 其他父容器宽度：子盒 width 按比例缩放即可（如父 780 → (780−24−24)/4 = 183）。**超界判断**（子总宽是否挤出）用 §6.13 的"父宽−4 阈值"，本条只算 width。

**关键认知**：撑满与否取决于**每行的列数 N**（即 grid-columns 值），不是子盒总数。子盒总数 > N 时会换行（见 6.10），每行宽度由 N 决定。
