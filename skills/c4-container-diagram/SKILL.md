...
| 4   | **对称**         | 子容器到父容器**左右边框距离相等**（水平居中）——**铁律**。垂直方向仅单列容器（竖条/标签列）要求上下均匀，普通多列容器不做垂直居中（D2 限制） |
...
| 对称（水平）      | **每个子容器显式 `width`**：`(层宽 − 24 − (N−1)×gap) / N`；竖条子容器 `width = 竖条宽 − 120`                                                                | 见 [§6.13](#613-尺寸计算规则父容器--子容器核心约束)                              |
| 对称（垂直）      | ⚠️ **D2/ELK 无法垂直居中**（grid 强制列等高、子容器靠顶部）。仅单列竖条/标签列可上下均匀：一维 `grid-columns:1` + 不写 `grid-gap` + 子容器按公式设 `height` | 见 [§6.13](#613-尺寸计算规则父容器--子容器核心约束)，普通多行容器不做垂直居中    |
| 均匀（等宽等高）  | 同分区子容器 width/height 一致 + grid-gap 统一                                                                                                              | 见 [§6.11](#611-同一列等宽约束)                                                  |
| 边界（不超界）    | width/height 精确按公式算；竖条宽 ≥ 最宽 label + 120；子容器 height 算小勿算大                                                                              | 见 [§6.13](#613-尺寸计算规则父容器--子容器核心约束)                              |
...
> **每个子容器在父容器内都必须有合理 Padding**：**左右留白必须相等**（水平居中）——这是硬性铁律；**上下留白**仅单列容器（竖条/标签列）要求均匀（D2/ELK 无法垂直居中，见 §1.3 铁律 2）。Padding 大小由父容器整体尺寸动态计算——不能写死，不能让子容器"贴左/贴右"或"撑满父容器"。

**三条铁律**：

1. **左右 Padding 等宽**（= 6.13 公式的 `width` 居中）：子容器 `width = (父宽 − 内边距 − (N−1)×gap) / N`。
2. **上下 Padding**（仅单列竖条/标签列适用，动态计算）：父容器有 label 时顶部留 ~36，底部留 ~14；无 label 时上下等大（如各 14）。子容器 `height = (父高 − 顶部label − 底部pad − (N−1)×gap) / N`。**普通多行/多列容器不做垂直居中**（D2 限制）。
...
> 完整版见 [6.13 尺寸计算规则](#613-尺寸计算规则父容器--子容器核心约束)（含确定性公式/垂直分布/边界约束/trade-off）。本条为基础公式 + 速查表。
...
> **这套流程解决"为什么有的行有的不行"**——所有踩过的坑都固化成确定性规则，写代码前算好、写代码后验证，不再靠试错。
...
3. **按 §4.8 给每个节点挂圆角 class**（border-radius）。
...
### 7.1 SVG 坐标验证（判断超界/等宽 — 落地脚本）

**有真实脚本，直接运行**（不靠识图描述）：

```bash
python3 scripts/verify-svg.py <渲染出的.svg>
# 输出: 每对父容器→直接子容器的 4 边超界数值 + 单列容器左右等宽判定
# 超界 >0 或 单列不等宽 → 脚本报 FAIL 并 exit 1
```
...
**判断结论**：直接给用户 PASS/FAIL + 数值。超界/不等宽 → 对照 §6.13 公式重算 width，再渲染重跑脚本。**脚本是权威，识图（7.3）只做辅助**。
