...
**默认直接执行** `remote-shell` 命令即可（绝大多数情况已安装）。**不要**每次先 `command -v remote-shell` 预检——那是浪费 token。仅当执行**失败**时才按其退出码走降级/提示：

| 执行结果 / 退出码                | 场景                   | 处理                                                |
| -------------------------------- | ---------------------- | --------------------------------------------------- |
| 退出码 0                         | 成功                   | 直接返回结果                                        |
| `command not found` / 退出码 127 | CLI 未安装 / 不在 PATH | 走下方「退出码获取与降级回退机制」表 126/127 行处理 |
| 其他非 0 退出码                  | 主机/认证/连接问题     | 走下方「退出码获取与降级回退机制」表处理            |
...
>
> CLI 缺失造成失败后，用户装好并配置主机，重新调用本 skill 即自动放行，无需修改 skill。

## 使用说明

使用 `remote-shell` 命令在指定的远程主机上执行命令。
...
```bash
# 上传：本地文件 → 远程主机
remote-shell upload <别名> <本地路径> <远程路径>

# 下载：远程主机文件 → 本地
remote-shell download <别名> <远程路径> <本地路径>
```
...
> ⚠️ 上方的 `Available hosts` 仅为示意，实际以 `remote-shell list` 为准。
...
- **禁止在 skill 内容、示例或对话中明文写入任何真实密码**；密码仅按上方「密码来源（本机密码表）」读取使用，不展示、不回显
...
> ⚠️ 示例 alias（`3070`、`dev`）仅为示意，实际可用别名以 `remote-shell list` 输出为准（同「执行命令」节声明）。
