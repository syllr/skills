...
| 特性                  | claude-reflect-system | singularity-claude     | self-improving-agent | OpenClaw  | OpenCode |
|---------------------|-----------------------|------------------------|----------------------|-----------|----------|
| **自改进方式**           | 用户纠正学习                | 自动评分 + 递归进化            | 全技能经验 + 用户反馈        | 网关协同执行    | 原生支持技能   |
| **完整生命周期**          | ❌ 仅限改进现有技能            | ✅ 缺口检测→创建→固化           | ✅ 所有技能持续进化          | ✅ 作为网关配合  | ✅ 加载技能   |
| **成熟度模型**           | ❌ 无                   | ✅ Draft → Crystallized | ✅ 置信度跟踪             | -         | -        |
| **量化评分**            | ❌ 置信度分级               | ✅ 5维度 0-100            | ✅ 用户评分(1-10)+置信度    | -         | -        |
| **自动修复**            | 用户触发                  | 低分自动触发                 | 错误钩子触发              | 需要配合      | 需要配合     |
| **多内存架构**           | ❌ 无                   | ❌ 无                    | ✅ 语义+情景+工作          | -         | -        |
| **遵循 Agent Skills** | ✅ Yes                 | ⚠️ 插件格式                | ✅ Yes                | ✅ 网关+技能兼容 | ✅ 原生支持   |
| **跨平台**             | ❌ Claude Code 仅       | ❌ Claude Code 仅        | ❌ Claude Code 仅      | ✅ 多平台     | ✅ 多平台    |
